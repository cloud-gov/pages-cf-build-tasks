...
async function renderFromTemplate(renderData, output, templateDir, templateName) {
  const templatePath = path.join(templateDir, templateName);

  const template = await fs.readFile(templatePath, 'utf8');
  const html = await ejs.render(template, { ...renderData, utils }, { filename: `${templateDir}/${templateName}` })
...
await renderFromTemplate(results, output, "templates", "report.ejs").then(console.log(`Report generation complete; open ${output} to review.`))
